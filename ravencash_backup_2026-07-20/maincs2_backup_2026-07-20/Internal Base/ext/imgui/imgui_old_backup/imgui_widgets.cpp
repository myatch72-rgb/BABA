...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.hovered_anim = ImLerp(it_anim->second.hovered_anim, hovered ? 1.0f : 0.0f, 0.05f * (1.0f - ImGui::GetIO().DeltaTime));

    
    RenderNavHighlight(bb, id);

    ImColor col = (held ? ImColor(accent_colour[0] - 0.05f, accent_colour[1] - 0.05f, accent_colour[2] - 0.05f, content_animation) : ImColor(accent_colour[0], accent_colour[1], accent_colour[2], content_animation));
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    window->DrawList->AddShadowRect(bb.Min, bb.Max, ImColor(accent_colour[0], accent_colour[1], accent_colour[2], it_anim->second.hovered_anim), 35.0f, ImVec2(1, 0), ImDrawFlags_ShadowCutOutShapeBackground, style.FrameRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float spacingx = style.ItemInnerSpacing.x - 5; 
    const float spacingy = style.ItemInnerSpacing.y - 20; 
    const ImVec2 pos = window->DC.CursorPos + ImVec2(square_sz + spacingx, square_sz + spacingy); 
    ...
    if (*v)
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(4, 3), ImColor(174, 139, 148, int(255 * ImGui::GetStyle().Alpha)), square_sz - 4);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect total_bb(bb.Min, bb.Max + ImVec2(0, 8));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRect(bb.Min, ImVec2(value_x2, bb.Max.y), ImColor(50, 50, 50), 5, ImDrawFlags_RoundCornersAll);
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(bb.Min.x + 5, bb.Min.y - style.FramePadding.y - 20), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(20, 18), window->DC.CursorPos + ImVec2(w + 90, label_size.y + style.FramePadding.y * 2.0f + 15));
    ...
    window->DrawList->AddRect(frame_bb.Min + ImVec2(- 5, - 5), frame_bb.Min + ImVec2(284, 18), ImColor(50, 50, 50, int(255 * ImGui::GetStyle().Alpha)), 3.0f);

    window->DrawList->AddRect(frame_bb.Min + ImVec2(-5, -5), frame_bb.Min + ImVec2(284, 18), ImColor(75, 75, 75, int(255 * it_filled->second)), 3.0f);
    ...
    RenderText(ImVec2(frame_bb.Max.x - label_buffer_size.x, frame_bb.Min.y - g.Font->FontSize - 7), value_buf, value_buf_end);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - style.FramePadding.y - 25), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() + 25, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); 
    const ImVec2 total_size = ImVec2(frame_size.x, frame_size.y + 15);

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 15), window->DC.CursorPos + frame_size + ImVec2(0, 15));
    ...
    static std::map <ImGuiID, input_text_anim> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end())
    {
        anim.insert({ id, {0.7f, 0.f} });
        it_anim = anim.find(id);
    }

    it_anim->second.label_anim = ImLerp(it_anim->second.label_anim, IsItemActive() ? 0.4f : (hovered ? 0.2f : 0.0f), 0.04f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.hovered_anim = ImLerp(it_anim->second.hovered_anim, IsItemActive() ? 0.9f : (hovered ? 0.2f : 0.0f), 0.08f * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    if (!is_multiline)
    {
        ...
        draw_window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(accent_colour[0], accent_colour[1], accent_colour[2], content_animation * 0.1f + it_anim->second.hovered_anim), style.FrameRounding);
    }
    ...
    if (label_size.x > 0) {
        PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_Text, 0.6f + it_anim->second.label_anim));
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddText(poppins, 17, pos + ImVec2(10, 8), ImColor(105, 105, 105, int(255 * ImGui::GetStyle().Alpha)), name);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(10, 10, 10)));
        PushStyleColor(ImGuiCol_Border, ImVec4(accent_colour[0], accent_colour[1], accent_colour[2], accent_colour[3]));
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    draw_list->AddCircleFilled(sv_cursor_pos, 5.f, ImColor(0, 0, 0));
    draw_list->AddCircleFilled(sv_cursor_pos, 4.f, ImColor(255, 255, 255));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 4 * it_active->second, bb.Max.y), ImColor(94, 159, 255, int(255 * it_active->second)), 10.f, ImDrawCornerFlags_Right);
