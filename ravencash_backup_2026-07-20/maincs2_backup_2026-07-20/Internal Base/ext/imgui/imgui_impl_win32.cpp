...
extern float g_BackBufferWidth;
extern float g_BackBufferHeight;
extern float g_WindowWidth;
extern float g_WindowHeight;

static void ScaleMousePos(float& x, float& y)
{
    if (g_BackBufferWidth > 0 && g_WindowWidth > 0 &&
        (g_BackBufferWidth != g_WindowWidth || g_BackBufferHeight != g_WindowHeight))
    {
        x = x * (g_BackBufferWidth / g_WindowWidth);
        y = y * (g_BackBufferHeight / g_WindowHeight);
    }
}
...
static void ImGui_ImplWin32_UpdateMouseData()
{
    ...
    if (is_app_focused)
    {
        ...
        if (!io.WantSetMousePos && bd->MouseTrackedArea == 0)
        {
            ...
            {
                float mpx = (float)pos.x, mpy = (float)pos.y;
                ScaleMousePos(mpx, mpy);
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NCMOUSEMOVE:
    {
        ...
        float mpx = (float)mouse_pos.x, mpy = (float)mouse_pos.y;
        ScaleMousePos(mpx, mpy);
